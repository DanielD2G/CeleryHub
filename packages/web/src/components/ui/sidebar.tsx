...
function SidebarProvider({
  ...
  const [sidebarWidth, _setSidebarWidth] = React.useState(() => {
    try {
      ...
      if (stored) return Math.min(SIDEBAR_WIDTH_MAX, Math.max(SIDEBAR_WIDTH_MIN, Number(stored)))
    } catch { /* ignore */ }
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  const handlePointerDown = React.useCallback(
    (e: React.PointerEvent) => {
      const wrapper = (e.target as HTMLElement).closest("[data-slot='sidebar-wrapper']")
      const current = wrapper
        ? parseInt(getComputedStyle(wrapper).getPropertyValue("--sidebar-width"), 10)
  ...
  return (
    <button
      ...
      className={cn(
        "hover:after:bg-sidebar-border absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] after:transition-colors after:duration-200 sm:flex",
