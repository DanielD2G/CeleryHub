...
async def _run_control(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail=f"Control action '{action}' failed: {e}",
        ) from e


@router.post("/pool-grow", response_model=ControlResponse)
async def pool_grow(req: PoolResizeRequest) -> ControlResponse:
...
async def pool_shrink(req: PoolResizeRequest) -> ControlResponse:
    return await _run_control(
        "pool_shrink",
...
async def rate_limit(req: RateLimitRequest) -> ControlResponse:
    return await _run_control(
        "rate_limit",
...
async def add_consumer(req: ConsumerRequest) -> ControlResponse:
    return await _run_control(
        "add_consumer",
...
async def cancel_consumer(req: ConsumerRequest) -> ControlResponse:
    return await _run_control(
        "cancel_consumer",
...
async def shutdown_workers(req: ShutdownRequest) -> ControlResponse:
    return await _run_control(
        "shutdown",
...
async def purge() -> ControlResponse:
    loop = asyncio.get_running_loop()
    try:
