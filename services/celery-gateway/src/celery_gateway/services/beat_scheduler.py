        ...
        async with get_session() as session:
            ...
            stmt = select(BeatSchedule).where(
                and_(
                    ...
                    BeatSchedule.next_run_at <= now_iso,
                )
            )
            ...
            for beat in due_beats:
                scheduled_at: str = beat.next_run_at or now_iso
                ...
                await session.execute(
                    update(BeatSchedule)
                    .where(BeatSchedule.id == beat.id)
                    .values(last_run_at=datetime.now(timezone.utc).isoformat())
                )
...
def stop_scheduler(task: asyncio.Task[None]) -> None:
    task.cancel()
