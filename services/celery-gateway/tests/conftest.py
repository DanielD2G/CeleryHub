...
async def db_session(
    ...
    with patch("celery_gateway.db.get_session", _override_get_session):
        with patch("celery_gateway.routers.beats.get_session", _override_get_session):
            async with factory() as session:
                yield session
...
async def fake_redis() -> AsyncGenerator[Any, None]:
    ...
    with patch(
        "celery_gateway.services.redis_client.get_redis", return_value=redis
    ):
        with patch(
            "celery_gateway.services.celery_redis.get_redis", return_value=redis
        ):
            yield redis
    await redis.aclose()
...
async def client(
    ...
    with patch("celery_gateway.main.celery_app", mock_celery_app):
        with patch("celery_gateway.routers.tasks.celery_app", mock_celery_app):
