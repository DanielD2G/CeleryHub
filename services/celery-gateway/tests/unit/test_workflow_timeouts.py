"""Tests for workflow node timeout logic (node-model)."""
...
def _make_workflow(session: AsyncSession, *, timeout: int | None = None) -> str:
    """Create a single-node workflow and return its ID."""
...
class TestNodeTimeout:
    async def test_no_timeout_does_not_schedule_task(
        ...
        """Node without timeout should not create a timeout asyncio.Task."""
        ...
        # No timeout tasks should exist for any node run in this run
        result = await db_session.execute(
            select(NodeRun).where(NodeRun.workflow_run_id == run_id)
        )
        for nr in result.scalars().all():
            assert nr.id not in _timeout_tasks

    async def test_timeout_schedules_task(
        self, db_session: AsyncSession
    ) -> None:
        """Node with timeout should create an asyncio.Task in _timeout_tasks."""
        ...
        node_run = result.scalar_one()
        assert node_run.id in _timeout_tasks
        assert not _timeout_tasks[node_run.id].done()

        # Cleanup
        _timeout_tasks[node_run.id].cancel()
        _timeout_tasks.pop(node_run.id, None)

    async def test_natural_completion_cancels_timeout(
        self, db_session: AsyncSession
    ) -> None:
        """When a node completes naturally, its timeout task should be cancelled."""
        wf_id = _make_workflow(db_session, timeout=300)
        await db_session.commit()
        ...
        node_run = result.scalar_one()
        assert node_run.id in _timeout_tasks
        timeout_task = _timeout_tasks[node_run.id]

        # Simulate task completion
        await on_task_completed("task-uuid-1", "SUCCESS")

        assert timeout_task.cancelled()
        assert node_run.id not in _timeout_tasks
    ...
    async def test_timeout_fires_marks_node_failed(
        self, db_session: AsyncSession
    ) -> None:
        """When timeout fires, node should be marked failed with timeout error."""
        wf_id = _make_workflow(db_session, timeout=300)
        await db_session.commit()
        ...
        _timeout_tasks.pop(node_run_id).cancel()

        await _expire_node(node_run_id, run_id, 300)
        ...
        assert "timed out after 300s" in (nr.error or "")
    ...
    async def test_timeout_advances_workflow(
        ...
        node_run = result.scalar_one()
        _timeout_tasks.pop(node_run.id).cancel()

        await _expire_node(node_run.id, run_id, 300)
    ...
    async def test_timeout_on_already_completed_node_is_noop(
        self, db_session: AsyncSession
    ) -> None:
        """If node already completed when timeout fires, nothing should happen."""
        wf_id = _make_workflow(db_session, timeout=300)
        await db_session.commit()
